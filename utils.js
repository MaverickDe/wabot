...
function extractMarketInfo(message) {
  ...
  let intent = "unknown";
  if (
    /(\b(wan\s*sell|want\s*sell|want\s*to\s*sell|for\s*sale|selling|i\s*dey\s*sell))\b/.test(
      lower
    )
  ) {
    intent = "sell";
  } else if (
    /(\b(wan\s*buy|looking\s*for|need|i\s*dey\s*find))\b/.test(lower)
  ) {
    intent = "buy";
  }
  ...
  const locationMatch = message.match(
    /\b(?:in|at|around|from)\s+([A-Za-z ]+)/i
  );
  ...
  let item = message;
  item = item.replace(
    /(wan\s*sell|for\s*sale|selling|i\s*dey\s*sell|wan\s*buy|looking\s*for|need|i\s*dey\s*find)/gi,
    ""
  );
...
`,
  ...
  try {
    const raw = text.trim();
    const jsonClean = raw.replace(/^```json\s*|\s*```$/g, "");
...
async function addGroupMembertomem(lid) {
  //   const redisKey = `group:${groupLid}`;
  const redisKey = `mem:${lid}`;
...
function writeMessageToCSV(data) {
  ...
  const row = `"${data.timestamp}","${data.senderName}","${
    data.phoneNumber
  }","${data.message.replace(/"/g, '""')}","${data.groupName}","${
    data.chatType
  }"\n`;
      ...
      if (e.exists) {
        ...
        if (member) {
          ...
          if (needsUpdate) {
            ...
            await addGroupMembertomem(e.lid);
            await db
